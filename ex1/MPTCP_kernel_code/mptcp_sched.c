...
static int mptcp_subflow_queued(struct sock *sk, u32 max_tso_segs)
{
	...
	//I Added
    int return_value = queued + tcp_packets_in_flight(tp);
    /* Calculate the difference between cwnd and queued segments */
    int cwnd_queued_diff = tp->snd_cwnd - return_value;
    /* Log the congestion window value and the difference */
